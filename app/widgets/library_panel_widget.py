...
class LibraryPanelWidget(QWidget):
    ...
    def _build_content(self, layout: QVBoxLayout) -> None:
        ...
        self.folder_path.setPlaceholderText(
            "Select a media library or series..."
        )
        self.folder_path.setReadOnly(True)

        browse_button = QPushButton("Browse...")
        browse_button.setToolTip(
            "Select a media library or series folder."
        )
        ...
        destination_button.clicked.connect(
            self._select_media_library
        )

        destination_layout = QHBoxLayout()
        destination_layout.addWidget(
            self.media_library_path
        )
        destination_layout.addWidget(
            destination_button
        )
        ...
        apply_primary_button(
            self.import_series_button
        )
        self.import_series_button.setToolTip(
            "Import a series folder or a single movie folder."
        )
        self.import_series_button.clicked.connect(
            self._import_series
        )
    ...
    def set_media_library_path(
        ...
        self.media_library_path.setText(
            path
        )

    def selected_media_library_path(
        self,
    ) -> str:
    ...
    def set_importing(
        ...
        if importing:
            ...
            self.import_series_button.setText(
                "Importing..."
            )
    ...
    def _import_series(self) -> None:
        ...
        if series_path:
            self.import_series_requested.emit(
                series_path
            )